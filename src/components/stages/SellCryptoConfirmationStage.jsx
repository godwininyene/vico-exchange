...
const SellCryptoConfirmationStage = ({
  selectedCrypto,
  coinAmount,
  nairaAmount,
  selectedAccount,
  ...
  const orderDetails = {
    ...
    symbol: selectedCrypto?.coinName,
    amount: coinAmount.toFixed(8),
    totalPaid: nairaAmount,
    account:
      userAccounts.find((acc) => acc.number === selectedAccount)?.bank +
      " - " +
      userAccounts.find((acc) => acc.number === selectedAccount)?.number,
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 text-center">
      ...
      <p className="mt-2 text-gray-600 dark:text-gray-300">
        We've received your transaction of{" "}
        <span className="font-bold">
          {coinAmount.toFixed(8)} {selectedCrypto?.coinName}
        </span>{" "}
        and we're processing your payment.
      </p>
      ...
      <div className="mt-6 flex items-center justify-center bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
        <p className="text-yellow-700 dark:text-yellow-300">
          Your payment will be sent to your account within 1-2 hours after
          verification.
        </p>
      </div>
...
export default SellCryptoConfirmationStage;
