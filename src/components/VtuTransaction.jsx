...
import {
  ...
  FiEye,
  FiChevronRight,
  FiRefreshCw, // added for reversed status
} from "react-icons/fi";
...
const VtuTransaction = ({
  ...
  const getBeneficiaryLabel = () => {
    ...
    if (name.includes("cable") || name.includes("dstv") || name.includes("gotv") || name.includes("startimes")) {
      return `${transaction.beneficiary} (IUC / SmartCard No)`;
    }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "reversed":
        return (
          <div className="text-orange-600 dark:text-orange-400 flex items-center gap-1 text-sm font-medium mt-1">
            <FiRefreshCw size={16} /> Reversed
          </div>
        );
      default:
        return (
        ...
        <div className="flex justify-between items-start gap-3">
          <div className="flex items-start flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0 ml-3">
              <h3
                className={`font-medium text-gray-800 dark:text-white ${isCompact ? "truncate" : "whitespace-normal break-words"
                  }`}
              >
              ...
              {transaction.planLabel && (
                <p
                  className={`text-sm text-gray-500 dark:text-gray-400 mt-1 ${isCompact ? "truncate" : "whitespace-normal break-words"
                    }`}
                >
                  {transaction.planLabel}
                </p>
              ...
              <p
                className={`text-sm text-gray-500 dark:text-gray-400 mt-1 ${isCompact ? "truncate" : "whitespace-normal break-all"
                  }`}
              >
          ...
          <div className="flex flex-col items-end flex-shrink-0 gap-2">
            <p
              className={`font-semibold whitespace-nowrap ${transaction.status === "success"
